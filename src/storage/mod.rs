...
mod tests {
    ...
    #[test]
    fn mentable_get_all_should_work() {
        let store = MemTable::new();
        test_get_all(store);
    }

    // #[test]
    // fn mentable_get_iter_should_work() {
    //     let store = MemTable::new();
    //     test_get_iter(store);
    // }
    ...
    fn test_basi_interface(store: impl Storage) {
        ...
        // get 不存在的 key 或者 table 得到 None
        assert_eq!(Ok(None), store.get("t1", "hello1"));
        assert!(store.get("t2", "world1").unwrap().is_none());

        // contains 存在的 key 或者 table 得到 true 否则 false
